...
bool isDirIgnoredForExport(Directory dir) => File('${dir.path}${Platform.pathSeparator}_ignore_export').existsSync();
...
File? createExportForModule(Directory rootDir) {
  final exportFile = File('${rootDir.path}${Platform.pathSeparator}export.dart');
  ...
  final files = searchFilesToExport(rootDir)
      .map((e) => isFileIgnoredForExport(e) ? null : getSubPath(rootDir, e.path))
      .whereType<String>();
  exportFile
    ..createSync()
